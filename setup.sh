...
if ! command -v conda &> /dev/null; then
    ...
    if ! conda info --envs | grep -q '^comfy\s'; then
        log_message "Conda environment 'comfy' not found. Creating it... COMFY_DOCKER_WIN32 is set to: ${COMFY_DOCKER_WIN32}"
        if [ "${COMFY_DOCKER_WIN32}" = "true" ]; then
            conda create --prefix /home/comfy/.condaenvs/comfy python=${COMFY_DOCKER_PYTHON_VERSION} -y
        else
            conda create --name comfy python=${COMFY_DOCKER_PYTHON_VERSION} -y
        fi
